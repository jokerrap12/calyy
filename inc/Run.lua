...
clay = Token:match("(%d+)")
redis:set(clay..":VERSION",1)
redis:set(clay..":SUDO_ID:",SUDO_USER)
redis:set(clay..":DataCenter:",'German')
redis:set(clay..":UserNameBot:",BOT_User)
redis:set(clay..":NameBot:",BOT_NAME)
redis:hset(clay..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3zoro_INSTALL","Yes")
info = {} 
...
clay = Token:match("(%d+)")
our_id = tonumber(clay)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(clay..":UserNameBot:")
SUDO_ID = tonumber(redis:get(clay..":SUDO_ID:"))
SUDO_USER = redis:hgetall(clay..'username:'..SUDO_ID).username
version = redis:get(clay..":VERSION")
DataCenter = redis:get(clay..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..clay..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(clay..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(clay..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(clay..'group:add'..msg.chat_id_) then 
...
elseif msg.sender_user_id_ == 913812904 then 
msg.TheRank =  'مطور السورس'
msg.Rank = 1
elseif msg.sender_user_id_ == 1003237899 then 
msg.TheRank =  ' مطور السورس '
msg.Rank = 1
...
msg.Rank = 1
elseif redis:sismember(clay..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(clay..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(clay..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(clay..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(clay..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(clay..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(clay..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(clay..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(clay..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(clay..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(clay..':WELCOME_BOT'),[[★ مرحباً انا بوت ]]..redis:get(clay..':NameBot:')..[[ .
✫• اختـصاصـي حمايـة الـمجموعات
✫• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس زورو ⇐ @SOURCE_ZORO
...
if msg.GroupActive and redis:get(clay..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(clay..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(clay..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(clay..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.clay then 
if msg.text and ScriptFile.iclay then
for k, clay in pairs(ScriptFile.clay) do
local SearchText = msg.text:match(clay)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iclay(msg,{msg.text:match(clay)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",clay.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dclay then
if ScriptFile.dclay(msg) == false then
return false
end
...
if Plug.clay then 
if msg.text and Plug.iclay then
for k, clay in pairs(Plug.clay) do
local SearchText = msg.text:match(clay)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(clay..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(clay..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iclay(msg,{msg.text:match(clay)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",clay.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source clay")
end 

end
else
print("The File Script.lua Not Runing in The Source clay")
...
function tdcli_update_callback(data)
	...
	redis:srem(clay..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/abazaa/clay/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/abazaa/clay/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/abazaa/clay/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/abazaa/clay/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/abazaa/clay/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/abazaa/clay/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(clay..":SUDO_ID:")
redis:hset(clay..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(clay..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(clay..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(clay..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(clay..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(clay..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(clay..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(clay..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(clay..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(clay..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(clay..'group:add'..data.chat_.id_) then
	redis:set(clay..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(clay..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(clay..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(clay..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	clay..'group:name'..k,IDS.Title,
	clay..'num_msg_zoro'..k,5,
	clay..'group:add'..k,true,
	clay..'lock_link'..k,true,
	clay..'lock_spam'..k,true,
	clay..'lock_webpage'..k,true,
	clay..'lock_markdown'..k,true,
	clay..'lock_flood'..k,true,
	clay..'lock_bots'..k,true,
	clay..'mute_forward'..k,true,
	clay..'mute_contact'..k,true,
	clay..'mute_document'..k,true,
	clay..'mute_inline'..k,true,
	clay..'lock_username'..k,true,
	clay..'replay'..k,true
	)
	redis:sadd(clay..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(clay..'username:'..ID,'username',user)
	redis:sadd(clay..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(clay..'username:'..ID,'username',user)
	redis:sadd(clay..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(clay..'username:'..ID,'username',user)
	redis:sadd(clay..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(clay..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(clay..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(clay..'username:'..data.user_.id_).username
	...
	redis:hset(clay..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
